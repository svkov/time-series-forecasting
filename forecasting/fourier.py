...
class Fourier(Model):
    ...
    def __init__(self, df_train: pd.DataFrame, n_harm=None, trend_deg=None, n=14, train_len=None, verbose=False,
        ...
        self.check_price_in_df(df_train)
        self.check_trend_deg(trend_deg)
        self.n = n
        self.df_train = df_train[['price']].interpolate().dropna()

        self.predictor = FourierPredictor(self.df_train, self.n)

        self.train_len = self.preproc_train_len(train_len)
        self.params = self.select_best_params(n_harm, trend_deg)

    def predict(self, df):
        self.check_price_in_df(df)
        return self.predictor.predict(df['price'].values, **self.params)
    ...
    def predict_for_report(self, df, date_start, date_end):
        self.check_price_in_df(df)
        predictor = FourierPredictor(df, self.n)
    ...
    def get_tests(self):
        ...
        for pivot in self.train_date_range:
            test = self.df_train.loc[:pivot, 'price'].values[-self.n:]  # последние n значений
...
class FourierPredictor:
    ...
    def get_prediction_by_pivot(self, pivot, **params):
        train = self.df_train.loc[:pivot, 'price'].values[:-self.n]  # все, кроме последних n значений
        if len(train) == 0:
